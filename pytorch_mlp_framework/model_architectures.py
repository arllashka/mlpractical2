...
class ConvolutionalProcessingBlockBN(nn.Module):
    ...
    def build_module(self):
        ...
        print(f"Output shape after build_module: {out.shape}")
...
class ConvolutionalDimensionalityReductionBlockBN(nn.Module):
    ...
    def build_module(self):
        ...
        # Dimensionality Reduction (Average Pooling)
        out = F.avg_pool2d(out, self.reduction_factor)
...
class ConvolutionalProcessingBlockBNRes(nn.Module):
    ...
    def build_module(self):
        ...
        in_channels = out.shape[1]
        if in_channels != self.num_filters:
            # Adjust input dimensions to match output dimensions for residual connection
            self.layer_dict['adjust'] = nn.Conv2d(
            ...
            out = self.layer_dict['adjust'](out)
        else:
            self.layer_dict['adjust'] = nn.Identity()
    ...
    def forward(self, x):
        identity = self.layer_dict['adjust'](x)

        out = self.layer_dict['conv_0'](identity)
        out = self.layer_dict['bn_0'](out)
        out = F.leaky_relu(out)
...
class ConvolutionalDimensionalityReductionBlockBNRes(nn.Module):
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation, reduction_factor):
        super(ConvolutionalDimensionalityReductionBlockBNRes, self).__init__()
